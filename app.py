...
def extract():
    ...
    try:
        ...
        content.append({'type': 'text', 'text': f'''Extract every field from this tax invoice. Return ONLY valid JSON — no markdown, no explanation:
...
- mrp and rate: numbers only (no units like /Nos)
- discount: percentage number only (no % symbol)
- Do NOT include subtotal, tax summary or grand total rows
- filename: {filename}'''})
        ...
        for l in parsed.get('lines', []):
            if not l.get('description', '').strip():
                continue
            rows.append({
                ...
                'buyer_fssai': parsed.get('buyer_fssai', ''),
